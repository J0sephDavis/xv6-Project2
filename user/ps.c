...
void print_as_table(int inuse, int pid, int tickets, int ticks, int priority) {
	printf(1, "   %d  |   %d  |   %d   |  %d   |  %d\n",
		inuse,
		pid,
		tickets,
...
void print_as_csv(int inuse, int pid, int tickets, int ticks, int priority) {
	printf(1, "%d,%d,%d,%d,%d\n",
		inuse,
		pid,
		tickets,
...
int main(int argc, char* argv[]) {
	...
	setpriority(40); 		//give the program a pretty high priority so that we get our output faster
	...
	printf(1," used | pid  |tickets| ticks | priority\n");
